            ...
            public void confirmMenu(Player player, int select) {
                switch (player.idMark.getIndexMenu()) {
                    ...
                    case ConstNpc.USE_THOI_VANG -> {
                        ...
                        long cost = costPerItem * sltv;

                        Item thoiVang = InventoryService.gI().findItemBag(player, 457);
                        if (thoiVang == null || thoiVang.quantity < sltv) {
                            Service.gI().sendThongBao(player, "Bạn không đủ " + sltv + " Thỏi vàng để bán!");
                            break;
                        }

                        long maxGold = nro.models.player.Inventory.LIMIT_GOLD;
                        if (player.inventory.gold > maxGold - cost) {
                            long remaining = maxGold - player.inventory.gold;
                            int maxSellable = (int) Math.min(thoiVang.quantity, remaining / costPerItem);
                            if (maxSellable < 1) {
                                Service.gI().sendThongBao(player, "Bạn đã đạt giới hạn vàng, không thể bán thêm!");
                            } else {
                                Service.gI().sendThongBao(player,
                                        "Bạn chỉ có thể bán tối đa " + maxSellable + " Thỏi vàng lúc này!");
                            }
                            break;
                        }
                        ...
                        Service.gI().sendThongBao(player,
                                "Đã bán " + sltv + " Thỏi vàng, nhận " + Util.numberToMoney(cost) + " vàng.");
                    }
