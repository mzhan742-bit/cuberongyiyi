...
public class Input {
    ...
    public static final int TRADE_RUBY_YIYI = 5102;
    ...
    public void doInput(Player player, Message msg) {
        try {
            ...
            switch (player.idMark.getTypeInput()) {
                ...
                case TRADE_GOLD -> {
                    YiYiPaymentService.gI().convertFlexibleToGoldBar(player, text[0]);
                }

                case TRADE_GEM -> {
                    YiYiPaymentService.gI().convertFlexibleToGem(player, text[0]);
                }

                case TRADE_RUBY_YIYI -> {
                    YiYiPaymentService.gI().convertGoldBarToRuby(player, text[0]);
                }
    ...
    public void createFormTradeGold(Player pl) {
        YiYiPaymentService.gI().refreshBalances(pl);
        createForm(pl, TRADE_GOLD,
                "Đổi Thỏi Vàng (Không khuyến cáo sử dụng tính năng này)\n"
                + "1.000 VNĐ = 5 Thỏi vàng\n"
                + "Số dư hiện tại: " + pl.getSession().vnd + " VNĐ",
                new SubInput("Nhập số tiền VNĐ muốn đổi", NUMERIC));
    }

    public void createFormTradeGem(Player pl) {
        YiYiPaymentService.gI().refreshBalances(pl);
        createForm(pl, TRADE_GEM,
                "Đổi Ngọc Xanh (Không khuyến cáo sử dụng tính năng này)\n"
                + "1.000 VNĐ = 30 Ngọc xanh\n"
                + "Số dư hiện tại: " + pl.getSession().vnd + " VNĐ",
                new SubInput("Nhập số tiền VNĐ muốn đổi", NUMERIC));
    }

    public void createFormTradeRubyFromGoldBar(Player pl) {
        createForm(pl, TRADE_RUBY_YIYI,
                "Đổi Hồng Ngọc\n1 Thỏi vàng = 100 Hồng ngọc",
