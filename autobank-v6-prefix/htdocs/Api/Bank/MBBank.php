 ...
 * SePay API v2 / OCB
 *
 * Thực tế giao dịch OCB/SePay hiện đang trả nội dung dạng:
 *   QR - lycafethu<ID>
 * Source web cũ lại tạo memo:
 *   cauberongyiyi<ID>
 *
 * V6 chấp nhận CẢ HAI prefix để không bỏ sót giao dịch cũ/mới.
 * Không dùng q-filter ở API nữa; lấy incoming OCB gần nhất rồi lọc bằng PHP.
 */
class MBBank
{
    private $apiKey;
    ...
    public function getTransactionHistory()
    {
        try {
            ...
            if ($httpCode !== 200) {
                throw new Exception(
                    $data['message'] ?? ('SePay HTTP ' . $httpCode)
                );
            }
    ...
    public function isYiYiTransaction($description)
    {
        $description = (string)$description;

        foreach (self::PREFIXES as $prefix) {
            if (stripos($description, $prefix) !== false) {
                return true;
            }
        }
    ...
    public function processDonateTransactions()
    {
        ...
        foreach ($history['data'] as $transaction) {
            ...
            if (!$this->isYiYiTransaction($description)) {
                continue;
            }
