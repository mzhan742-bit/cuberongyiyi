...
if (
    !isset($_GET['key']) ||
    !hash_equals($CRON_KEY, (string)$_GET['key'])
) {
...
echo '<h2>TEST SEPAY AUTOBANK - TEAM2026 V6</h2>';
echo '<p><b>Database:</b> '
    . htmlspecialchars($servers[$currentServer]['name'])
    . '</p>';
echo '<p style="color:#b65d2f"><b>'
    . 'Trang này chỉ kiểm tra. AutoCron chạy nền mới là nơi cộng tiền.'
    . '</b></p>';
echo '<p><b>Prefix chấp nhận:</b> lycafethu + cauberongyiyi</p>';
...
echo '<p>Tổng giao dịch API trả về: <b>'
    . count($history['data'])
    . '</b></p>';
...
foreach ($matched as $transaction) {
    $accountId = $mbBank->extractUsernameFromDescription(
